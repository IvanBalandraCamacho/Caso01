...
export default function GeneralChatPage({
  ...
  // Efecto para enviar mensaje inicial cuando hay un workspace activo
  useEffect(() => {
  ...
  }, [initialMessage, messages.length, chatId, router, activeWorkspace?.id])
  ...
  const handleSendMessageWithStreaming = async (messageText: string) => {
    ...
    // Verificar que hay un workspace activo
    if (!activeWorkspace?.id) {
      message.error("Por favor selecciona un workspace primero")
      return
    }
      ...
      >
        ...
        >
          <div style={{ width: "100%", maxWidth: "800px" }}>
            {/* Mostrar advertencia si no hay workspace seleccionado */}
            {!activeWorkspace && messages.length === 0 && (
              ...
              >
                <Text style={{ color: "#E3E3E3", fontSize: "14px" }}>
                  ⚠️ Por favor selecciona un workspace en el panel lateral para poder chatear.
                </Text>
              </div>
            )}
